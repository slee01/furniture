...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, params):
        ...
        self.discrete_act = [[-3.0, 0.0, 3.0], [-0.2, 0.0, 0.2]]  # acc, steer
        ...
        if self.discrete:
            ...
            self.action_space = spaces.Box(np.array([-3.0, -0.3]), np.array([3.0, 0.3]), dtype=np.float32)  # acc, steer
        ...
        self.world = client.load_world(params['town'])  # change map shown in server
        print('Carla server connected!')

        # Set weather
        self.world.set_weather(carla.WeatherParameters.ClearNoon)  # change weather shown in server

        # Get spawn points
        self.vehicle_spawn_points = list(self.world.get_map().get_spawn_points())  # component type is Transform object(defining a location and rotation for the actor)
        self.walker_spawn_points = []
        for i in range(self.number_of_walkers):
            spawn_point = carla.Transform()
            loc = self.world.get_random_location_from_navigation()
			# Quite the same works goes for walkers, but this time the world can get a random point on a sidewalk
			# (this same method is used to set goal locations for walkers)
    ...
    def _get_actor_polygons(self, filt):
        ...
        for actor in self.world.get_actors().filter(filt):
            ...
            l = bb.extent.x  # half the size of the box for that axis
            w = bb.extent.y  # half the size of the box for that axis
